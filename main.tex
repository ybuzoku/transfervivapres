...
\begin{frame}{Notes on $\suppM{\baseB}{L}$}
	...
	\vspace{5pt}
	\item $\suppM{\baseB}{L}\varphi$  iff $\suppM{\baseB}{L}\varphi\mand\mtop$ iff $\suppM{\baseB}{L}\varphi\msetsum\mtop$
	\vspace{5pt}
...
\begin{frame}{Why the extension to Inf?}
	\begin{center}
		\textbf{Are we still inferentialist?}
	\end{center}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Including the exponential}
\begin{frame}{Can we understand the exponential better?}
\begin{itemize}	
...
\pause
\item This however fails to be complete as it requires us to be able to prove from $L \deriveBaseM{\baseB} \flatIMALL{(\bang \varphi)}$ that $L = \emptymultiset$, to pass Proposition 31 from the report.
\pause
...
\begin{frame}{Exploring the zoned semantics}
	\begin{definition}[Zoned basic derivability relation]
		...
		\item[App] Given that $(\openaddrule(P_{1_i};C_{1_i} \seq q_{1_{i}})\Closeaddrule{i=1}{l_1},\dots,\openaddrule(P_{n_i};C_{n_i} \seq q_{n_{i}}) \Closeaddrule{i=1}{l_n} \Rightarrow \at{r}) \in \baseB$ and atomic multisets $G$ and $\at{C}_i$ such that the following hold:
				\[G,P_{i_j};L_i, C_{i_j} \deriveBaseM{\baseB} \at{q}_{i_j} \text{ for all } i = 1,\dots, n \text{ and } j = 1,\dots,l_i\]
		Then $G;L_{1},\dots,L_{n}\deriveBaseM{\baseB} r$. 
		\end{itemize}
	\end{definition}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\vspace{0.3cm}
\item $\suppM{\baseB}{G;L} \bang\varphi$ iff for every base $\baseC \baseGeq \baseB$, atomic multiset $K$ and atom $p,$ such that $\varphi;\cdot \suppM{\baseC}{G; K} p$ hold, then $\suppM{\baseC}{G; L\msetsum K}p$.
\vspace{0.3cm}
